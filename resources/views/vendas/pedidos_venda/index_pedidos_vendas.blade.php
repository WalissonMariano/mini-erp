<div style="padding: 0 20px 20px 20px; background-color: #ffffff; min-height: 100vh; font-family: Arial, sans-serif;">
    ...
    <div class="header-bar">
        <div>
            <h2 style="margin: 0 0 4px 0;">Pedidos de venda</h2>
            <div class="subtitle">Grade com todos os pedidos cadastrados (PedidosVenda)</div>
        </div>
        <button onclick="loadContent('{{ route('pagina.novo.pedido_venda') }}')" class="new-btn" type="button" title="Novo pedido">
            + Novo pedido
        </button>
    </div>
        ...
        <table>
            ...
            <tbody>
                @forelse ($pedidosVenda as $pedido)
                    <tr>
                        <td>
                            <button onclick="loadContent('{{ route('pagina.editar.pedido_venda', ['id' => $pedido->id]) }}')" class="edit-btn" type="button" title="Editar pedido">
                                ✏️
                            </button>
                        </td>
                ...
                @empty
                    <tr>
                        <td colspan="8" style="text-align: center; color: #7f8c8d;">Nenhum pedido de venda cadastrado.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
