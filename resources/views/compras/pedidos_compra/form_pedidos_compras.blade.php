...
<div style="padding: 0 20px 20px 20px; background-color: #ffffff; min-height: 100vh; font-family: Arial, sans-serif;">
    ...
    <h2>{{ $isEdit ? 'Editar pedido de compra' : 'Novo pedido de compra' }}</h2>

    <div class="card">
        @if ($formAction === '#')
            <div class="alert alert-error">
                Rotas de salvar/atualizar pedido de compra nao foram localizadas.
            </div>
        @endif
        ...
        <form action="{{ $formAction }}" method="POST">
            ...
            <div class="card-header">
                <strong style="font-size: 18px; color: #2c3e50;">
                    {{ old('status', optional($pedidoCompra)->status ?? 'rascunho') }}
                    — {{ old('data_pedido', optional($pedidoCompra)->data_pedido?->format('Y-m-d') ?? now()->format('Y-m-d')) }}
                </strong>
            </div>

            <div class="section-title">Dados do pedido (pedidos_compras)</div>
            <div class="field-group three">
                <div class="field">
                    <label>Data do pedido</label>
                    <input type="date" name="data_pedido" required
                        value="{{ old('data_pedido', optional($pedidoCompra)->data_pedido?->format('Y-m-d') ?? now()->format('Y-m-d')) }}">
                </div>
                <div class="field">
                    <label>Status</label>
                    <input type="text" name="status" maxlength="20" required
                        value="{{ old('status', optional($pedidoCompra)->status ?? 'rascunho') }}"
            ...
            <div class="field-group full">
                <div class="field">
                    ...
                    <textarea name="str_observacao" maxlength="2000" placeholder="Observacoes do pedido...">{{ old('str_observacao', optional($pedidoCompra)->str_observacao ?? '') }}</textarea>
                </div>
            </div>

            @include('compras.pedidos_compra_itens.form_pedidos_compra_itens', [
                'catalogoItens' => $catalogoItens,
                'itensFormRows' => $itensFormRows,
            ])

            <div class="actions">
                <button class="btn btn-secondary" type="button" onclick="loadContent('{{ route('pagina.lista.pedidos_compra') }}')">Cancelar</button>
...
<script>
    ...
    (function () {
        ...
        function bindRemover(tr) {
            tr.querySelectorAll('.pc-remove-item-row').forEach(function (btn) {
        ...
        tbody.querySelectorAll('tr.pc-item-linha').forEach(bindRemover);
