...
<div style="padding: 0 20px 20px 20px; background-color: #ffffff; min-height: 100vh; font-family: Arial, sans-serif;">
    ...
    <h2>{{ $isEdit ? 'Editar conta a receber' : 'Nova conta a receber' }}</h2>

    <div class="card">
        @if ($formAction === '#')
            <div class="alert alert-error">Rotas de salvar/atualizar conta a receber nao foram localizadas.</div>
        @endif
        ...
        <form action="{{ $formAction }}" method="POST">
            ...
            <div class="card-header">
                <strong style="font-size: 18px; color: #2c3e50;">
                    {{ old('str_descricao', optional($conta)->str_descricao ?: 'Conta a receber') }}
                </strong>
            </div>

            <div class="section-title">Dados (contas_receber)</div>

            <div class="field-group full">
                <div class="field">
                    <label>Descricao</label>
                    <input type="text" name="str_descricao" maxlength="255" value="{{ old('str_descricao', optional($conta)->str_descricao ?? '') }}" placeholder="Ex.: Parcela pedido Y">
                </div>
            </div>
            ...
            <div class="actions">
                <button class="btn btn-secondary" type="button" onclick="loadContent('{{ route('pagina.lista.contas_receber') }}')">Cancelar</button>
