<div style="padding: 0 20px 20px 20px; background-color: #ffffff; min-height: 100vh; font-family: Arial, sans-serif;">
    ...
    <div class="header-bar">
        <div>
            <h2 style="margin: 0 0 4px 0;">Contas a pagar</h2>
            <div class="subtitle">Titulos a fornecedores e demais despesas (contas_pagar)</div>
        </div>
        <button onclick="loadContent('{{ route('pagina.novo.conta_pagar') }}')" class="new-btn" type="button" title="Nova conta">
            + Nova conta
        </button>
    </div>
        ...
        <table>
            ...
            <tbody>
                @forelse ($contas as $conta)
                    <tr>
                        <td>
                            <button onclick="loadContent('{{ route('pagina.editar.conta_pagar', ['id' => $conta->id]) }}')" class="edit-btn" type="button" title="Editar">✏️</button>
                        </td>
                ...
                @empty
                    <tr>
                        <td colspan="8" style="text-align: center; color: #7f8c8d;">Nenhuma conta a pagar cadastrada.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
