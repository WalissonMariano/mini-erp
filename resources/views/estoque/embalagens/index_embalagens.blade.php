<div style="padding: 0 20px 20px 20px; background-color: #ffffff; min-height: 100vh; font-family: Arial, sans-serif;">
    ...
    <div class="header-bar">
        <div>
            <h2 style="margin: 0 0 4px 0;"> Cadastro de Embalagens</h2>
            <div class="subtitle">Grade com todas as embalagens cadastradas</div>
        </div>
        <button onclick="loadContent('{{ route('pagina.cadastro.embalagens') }}')" class="new-btn" type="button" title="Nova embalagem">
            + Nova Embalagem
        </button>
    </div>
        ...
        <table>
            ...
            <tbody>
                @foreach ($embalagens as $embalagem)
                    ...
                    <tr>
                        <td>
                            <button onclick="loadContent('{{ route('pagina.editar.embalagens', ['id' => $embalagem->id]) }}')" class="edit-btn" type="button" title="Editar embalagem">
                                ✏️
                            </button>
                        </td>
                        <td>{{ $embalagem->str_sigla }}</td>
                        <td>{{ $embalagem->str_descricao }}</td>
                        <td>{{ $embalagem->dbl_quantidade_embalagem }}</td>
                @endforeach
