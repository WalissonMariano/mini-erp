...
<div class="corp-shell">
    <style>
        ...
        .corp-embalagem-table {
            width: 100%;
            border-collapse: collapse;
            font-size: 0.875rem;
        }

        .corp-embalagem-table thead th {
            text-align: left;
            padding: 10px 14px;
            font-weight: 600;
            font-size: 0.6875rem;
        ...
        .corp-embalagem-table th.col-idx {
            width: 44px;
            text-align: center;
        }

        .corp-embalagem-table th.col-acoes {
            width: 100px;
            text-align: center;
        }

        .corp-embalagem-table tbody td {
            padding: 10px 14px;
            border-bottom: 1px solid var(--corp-border);
            vertical-align: middle;
        }

        .corp-embalagem-table tbody tr:last-child td {
            border-bottom: none;
        }

        .corp-embalagem-table tbody tr:hover td {
            background: #fafbfc;
        }

        .corp-embalagem-table td.col-idx {
            text-align: center;
            font-weight: 600;
            font-size: 0.8125rem;
            color: var(--corp-text-muted);
        ...
        .corp-embalagem-table td.col-acoes {
            text-align: center;
        }

        .corp-embalagem-table td select {
            min-height: 40px;
        }
    ...
    <div class="corp-form-card">
        ...
        <form id="corp-item-form" action="{{ $formAction }}" method="POST">
            ...
            <div class="corp-form-card__body">
                ...
                <div class="corp-section-title">Embalagens do item</div>

                @if ($embalagens->isEmpty())
                    <p class="corp-hint" style="margin-bottom: 0;">Nenhuma embalagem cadastrada. Cadastre embalagens no menu Estoque antes de vincular aqui.</p>
                @else
                    <div class="corp-embalagem-wrap">
                        <table class="corp-embalagem-table">
                            <thead>
                                <tr>
                                    <th class="col-idx">#</th>
                                    <th>Embalagem</th>
                            ...
                            <tbody id="embalagem-tbody">
                                @foreach ($linhasEmbalagem as $idx => $valorLinha)
                                    <tr class="embalagem-linha">
                                        <td class="col-idx embalagem-idx">{{ $loop->iteration }}</td>
                                        <td>
                                            <div class="corp-field corp-field--inline">
                                        ...
                                        <td class="col-acoes">
                                            <button type="button" class="corp-btn--danger embalagem-remover-linha" title="Remover linha">Remover</button>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
                    ...
                    <button type="button" class="corp-btn corp-btn--ghost" id="embalagem-add-linha">+ Adicionar linha</button>
                @endif
            </div>
        </form>
    </div>
