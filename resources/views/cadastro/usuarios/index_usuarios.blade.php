<div style="padding: 0 20px 20px 20px; background-color: #ffffff; min-height: 100vh; font-family: Arial, sans-serif;">
    ...
    <div class="header-bar">
        <div>
            <h2 style="margin: 0 0 4px 0;">👤 Cadastro de Usuarios</h2>
            <div class="subtitle">Grade com todos os usuarios cadastrados</div>
        </div>
        <button onclick="loadContent('{{ route('pagina.cadastro.usuarios') }}')" class="new-btn" type="button" title="Novo usuario">
            + Novo Usuario
        </button>
    </div>
        ...
        <table>
            ...
            <tbody>
                @foreach ($usuarios as $usuario)
                    ...
                    <tr>
                        <td>
                            <button onclick="loadContent('{{ route('pagina.editar.usuarios', ['id' => $usuario->id]) }}')" class="edit-btn" type="button" title="Editar usuario">
                                ✏️
                            </button>
                        </td>
                        ...
                        <td><span class="status">{{ $usuario->is_active ? 'Ativo' : 'Inativo' }}</span></td>
                    </tr>
                @endforeach
