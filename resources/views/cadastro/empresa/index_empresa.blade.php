<div style="padding: 0 20px 20px 20px; background-color: #ffffff; min-height: 100vh; font-family: Arial, sans-serif;">
    ...
    <div class="header-bar">
        <div>
            <h2 style="margin: 0 0 4px 0;">🏢 Cadastro de Empresas</h2>
            <div class="subtitle">Grade com todas as empresas cadastradas</div>
        </div>
        <button onclick="loadContent('{{ route('pagina.cadastro.empresa') }}')" class="new-btn" type="button" title="Nova empresa">
            + Nova Empresa
        </button>
    </div>
        ...
        <table>
            ...
            <tbody>
                @foreach ($empresas as $empresa)
                    ...
                    <tr>
                        <td>
                            <button onclick="loadContent('{{ route('pagina.editar.empresa', ['id' => $empresa->id]) }}')" class="edit-btn" type="button" title="Editar empresa">
                                ✏️
                            </button>
                        </td>
                        ...
                        <td><span class="status">{{ $empresa->status ? 'Ativa' : 'Inativa' }}</span></td>
                @endforeach
